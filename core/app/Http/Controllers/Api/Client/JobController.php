    ...
    public function job_create(Request $request)
    {
        if($request->isMethod('post'))
        {
            ...
            if ($attachment = $request->file('attachment')) {
                $request->validate([
                    'attachment'=>'required|mimes:png,jpg,jpeg,bmp,gif,tiff,svg,csv,txt,xlx,xls,pdf,docx',
            ...
            $job->job_skills()->attach(json_decode($request->skill,true));
              ...
              try {
            $job_link = url('/job/' . $job->slug);
            $message = '
                <p>A new job has been posted on <strong>RightFreelancer</strong>.</p>
                <p><strong>Title:</strong> ' . $job->title . '</p>
                <p><strong>View Job:</strong> <a href="' . $job_link . '">' . $job_link . '</a></p>
                <br>
                <p>Thank you for choosing <strong>RightFreelancer</strong>.<br>
                Best regards,<br>
                The RightFreelancer Team</p>
            ';
