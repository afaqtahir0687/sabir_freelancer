...
//backend
Route::group(['as'=>'admin.','prefix'=>'admin/subscription','middleware' => ['auth:admin','setlang',"check.subscription"]],function(){
...
//client subscription
Route::group(['prefix'=>'client/subscription','as'=>'client.','middleware'=>['auth','userEmailVerify','Google2FA','globalVariable', 'maintains_mode','setlang', 'check.subscription']],function() {
...
Route::group(['prefix'=>'freelancer/packages','as'=>'freelancer.','middleware'=>['auth','userEmailVerify','Google2FA','globalVariable', 'maintains_mode','setlang', 'check.subscription']],function() {
...
Route::group(['middleware' => ['globalVariable', 'maintains_mode','setlang', 'check.subscription']], function () {
    ...
    Route::controller(\Modules\Subscription\Http\Controllers\Frontend\FrontendSubscriptionController::class)->group(function(){
        Route::get('packages', 'subscriptions')->name('subscriptions.all');
        Route::get('packages/all', 'subscriptions')->name('subscriptions.all');
        Route::get('packages/all/pagination', 'pagination')->name('subscriptions.pagination');
        Route::get('packages/filter/type/wise', 'filter_subscriptions')->name('subscriptions.filter');
        Route::post('packages/user/login', 'user_login')->name('subscriptions.user.login');
        Route::post('packages/buy', 'buy_subscription')->name('subscriptions.buy');
        Route::post('subscriptions/downgrade', 'downgradeSubscription')->name('subscriptions.downgrade');
    });
});
