    ...
    public function buy_subscription(Request $request)
    {
        if(isset($request->subscription_id)){
            ...
            if(!$subscription_details) {
                return back()->with(toastr_warning(__('Invalid subscription details')));
            }
            ...
            UserSubscription::where('user_id', $user->id)->where('status', 1)->where('expire_date', '>', now())->update(['status' => 0]);
            ...
            else
            {
                ...
                $description = sprintf(__('Order id #%1$d Email: %2$s, Name: %3$s'),$subscription_id,$email,$name);
                ...
                elseif($request->selected_payment_gateway === 'payfast'){
                    try {
                        return PaymentGatewayRequestHelper::payfast()->charge_customer($this->buildPaymentArg($total,$title,$description,$subscription_id,$email,$name,$user_type,route('bs.payfast.ipn.subscription')));
                    }catch (\Exception $e){
                        toastr_error($e->getMessage());
                        return back();
                    }
                ...
                elseif($request->selected_payment_gateway === 'authorize_dot_net'){
                    try {
                        return PaymentGatewayRequestHelper::authorizenet()->charge_customer($this->buildPaymentArg($total,$title,$description,$subscription_id,$email,$name,$user_type,route('bs.authorize.ipn.subscription')));
                    }catch (\Exception $e){
                        toastr_error($e->getMessage());
                        return back();
                    }
