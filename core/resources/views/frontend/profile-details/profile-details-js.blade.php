           ...
           document.querySelector('#upload_portfolio_photo').addEventListener('change', function() {
                ...
                if (this.files && this.files[0]) {
                    ...
                    if (file.size > maxSizeBytes) {
                        toastr_warning_js("{{ __('Image must be less than 1MB') }}");

                        this.value = "";
                        document.querySelector('.portfolio_photo_preview').src = "";
                        $("#add_portfolio_form").find('.change_image_text').text("{{ __('Click to upload portfolio image') }}");
                        return;
                    }
                    ...
                    const img = document.querySelector('.portfolio_photo_preview');
                    img.onload = () => {
            ...
            document.querySelector('#change_portfolio_photo').addEventListener('change', function () {
                ...
                if (this.files && this.files[0]) {
                    ...
                    if (file.size > maxSizeBytes) {
                        ...
                        this.value = "";
                        document.querySelector('.edit_portfolio_photo_preview').src = "";
                        $("#edit_portfolio_form").find('.change_image_text').text("{{ __('Click to upload portfolio image') }}");
                        return;
                    }

                    const img = document.querySelector('.edit_portfolio_photo_preview');
                    img.onload = () => {
                        URL.revokeObjectURL(img.src); 
                    };
                    img.src = URL.createObjectURL(file);

                    $("#edit_portfolio_form").find('.change_image_text').text("{{ __('Click to change photo') }}");
                }
            });
