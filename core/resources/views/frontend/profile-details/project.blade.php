                    ...
                    <a href="{{ route('project.details', ['username' => $project->project_creator?->username, 'slug' => $project->slug]) }}">
                        ...
                        @if($projectImage)
                            @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                <img src="{{ render_frontend_cloud_image_if_module_exists('project/' . $projectImage, load_from: $project->load_from) }}" alt="project">
                            @else
                                @php
                                    $localPath = public_path('assets/uploads/project/' . $projectImage);
                                @endphp
                                @if(file_exists($localPath))
                                    <img src="{{ url('assets/uploads/project/' . $projectImage) }}" alt="project">
                                @else
