...
@section('site_title', __('Contact Us'))
@section('meta_title'){{ __('Contact Us') }}@endsection
@section('meta_description'){{ __('Get in touch with us through our contact page for any inquiries or support.') }}@endsection
...
@section('content')
    ...
    @if($page_post->page_builder_status === 'on')
        ...
        @if(!auth()->guard('web')->check() && $page_post->visibility === 'all')
            @include('frontend.pages.dynamic.partials.dynamic-page-builder-part',['page_post' => $page_post])
        ...
        @endif
    @else
        @include('frontend.pages.dynamic.partials.dynamic-content')
    @endif
@endsection
